...
**工作流程（v4.0 架构）**：
```
【管理员】领域描述/Mermaid图 → [本体管理] → 标准化知识本体 → 保存
                                    ↓
【用户】原始资讯 → [基于已有本体格式化] → 易读输出
                          ↑
                    只读本体图可视化
```

**角色分离**：
- **管理员**：通过本体管理界面构建和维护知识本体（Mermaid编辑 + AI生成）
- **用户**：输入内容，基于已有本体获得格式化输出（只读模式）

### 1.2 MVP 核心理念
用最小成本验证"基于预构建本体的智能资讯整理"这一核心价值，聚焦单用户场景，使用前端缓存存储。

**核心价值主张**：
- 管理员预先构建领域知识本体（工具）
- 用户输入原始资讯
- AI 基于已有本体整理出易读的内容（产品）
- 用户获得结构清晰的阅读体验
...
解析Mermaid代码为本体JSON
{
  ...
  "edges": [{source, target, relation, relation_type, strength}]
}
    ↓
更新本体
    ↓
...
```
管理员输入领域描述
（例如："基于你对于经济、贸易、政治、金融等的专业知识和历史事件，
构建一个本体架构作为你的大纲"）
    ↓
调用 AI buildOntology API
    ↓
基于 Meta Ontology 框架生成规范化本体
    ↓
转换为 Mermaid 代码显示在编辑器
    ↓
管理员预览/调整
    ↓
点击"应用到本体"
...
**Meta Ontology 框架约束**：
- 6个顶层领域（DOM_*）
- 5种实体类型（THEORY_CONCEPT, ALGORITHM_METHOD等）
- 9种核心关系（is-a, part-of, enables等）
...
用户输入资讯内容
    ↓
检查本体是否存在
  - 不存在 → 提示"请先在本体管理中构建知识本体"
  - 存在 → 继续
    ↓
调用 AI formatContent API
（输入：资讯内容 + 已有本体）
    ↓
AI基于本体整理格式化内容
    ↓
...
```
┌────────────────────────────────────────────────────────┐
│  Header: 🧠 Ontology Reader | 📊 本体管理 | ⚙️ 设置      │
├────────────────────────────────────────────────────────┤
│                                                        │
│  ┌────────────────────────────────────────────────┐   │
│  │          资讯输入区域（上半部分）               │   │
...
├── src/
│   ├── components/
│   │   ├── InputPanel.jsx           # 输入区域（仅格式化）
│   │   ├── OutputPanel.jsx          # 输出区域
│   │   ├── OntologyGraph.jsx        # 本体图（只读模式）
│   │   ├── OntologyManagement.jsx   # 本体管理（v4.0新增）
│   │   ├── VersionCompare.jsx       # 版本对比（已废弃）
│   │   └── SettingsModal.jsx        # 设置
│   ├── store/
│   │   └── useOntologyStore.js      # Zustand状态管理
│   ├── utils/
│   │   ├── storage.js               # LocalStorage 封装
│   │   ├── aiService.js             # AI 调用（Moonshot）
│   │   ├── ontologyDiff.js          # 版本对比算法（已废弃）
│   │   └── ontologyUtils.js         # 本体工具函数
│   ├── prompts/
...
**v4.0 文件变更**：
- ✅ 新增 `OntologyManagement.jsx`（Mermaid编辑器 + AI生成）
- ✅ 修改 `InputPanel.jsx`（移除本体构建/更新逻辑）
- ✅ 修改 `OntologyGraph.jsx`（改为只读模式）
- ✅ 新增依赖 `mermaid`（图语言渲染）
- ❌ 废弃 `VersionCompare.jsx`（不再使用）
- ❌ 废弃 `ontologyDiff.js`（不再使用）
```
...
- 检查本体是否存在（提示用户先构建）
- 基于已有本体格式化输入内容
- 仅调用 formatContent API（不修改本体）
- 处理速度更快、成本更低
- 输出一致性更好

### 13.3 Meta Ontology 框架 ✅
- 6个顶级知识领域分类（DOM_*）
- 5种规范化实体类型（THEORY_CONCEPT等）
- 9种核心关系词汇（is-a, part-of, enables等）
- 跨领域同构关系识别
- 自动清理无效边
...
**开发完成时间**：4 周（已完成）
**开发人数**：1 人
**技术栈**：React 18 + Vite + Zustand + React Flow + Tailwind CSS + Mermaid.js
...
**v4.0 (当前版本) - 架构优化**
- ✅ 新增本体后台管理界面（Mermaid编辑器 + AI生成）
- ✅ 分离管理员和用户工作流程
- ✅ 内容输入仅用于格式化（不再构建/更新本体）
- ✅ 主界面本体图改为只读模式
- ✅ 支持通过Mermaid图语法直接构建本体
- ✅ 支持通过AI从领域描述生成本体
